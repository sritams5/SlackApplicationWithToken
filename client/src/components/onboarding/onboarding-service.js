...
export async function saveUpdateUserAfterLogin(userUID, response)
{
  ...
  const token = decodeURIComponent(getUrlParameter('token')).trim();
    ...
    if (teamnameFromUrl != null && teamnameFromUrl != 'undefined' && teamnameFromUrl != "")
    {
      ...
      if (token != null && token != 'undefined' && token != ""){
        ...
        if(team != null && team != 'undefined' && team != "") {
          let invitations = JSON.stringify(team.invitations);
          if(invitations != null && invitations != 'undefined' && invitations != "") {
            console.log('invitations-'+invitations);
            if (invitations.indexOf(token)>-1) {
